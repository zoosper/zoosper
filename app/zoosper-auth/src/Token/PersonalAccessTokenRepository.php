...
final readonly class PersonalAccessTokenRepository
{
    ...
    public function touch(int $id,string $now,string $cutoff):void{$s=$this->pdo->prepare('UPDATE personal_access_tokens SET last_used_at=:last_used_at,updated_at=:updated_at WHERE id=:id AND (last_used_at IS NULL OR last_used_at<:cutoff)');$s->execute(['last_used_at'=>$now,'updated_at'=>$now,'id'=>$id,'cutoff'=>$cutoff]);}
