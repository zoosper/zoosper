...
final readonly class PersonalAccessTokenRepository
{
    ...
    public function touch(int $id,string $now):void{$s=$this->pdo->prepare('UPDATE personal_access_tokens SET last_used_at=:now,updated_at=:now WHERE id=:id');$s->execute(['now'=>$now,'id'=>$id]);}
    public function revoke(int $id,int $ownerId,string $now):bool{$s=$this->pdo->prepare('UPDATE personal_access_tokens SET revoked_at=:now,updated_at=:now WHERE id=:id AND admin_user_id=:owner AND revoked_at IS NULL');$s->execute(['now'=>$now,'id'=>$id,'owner'=>$ownerId]);return $s->rowCount()===1;}
