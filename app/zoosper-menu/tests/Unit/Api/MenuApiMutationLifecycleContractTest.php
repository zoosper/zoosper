...
it('exposes stateless shared Menu mutations lifecycle and guarded deletion',function():void{
 $root=dirname(__DIR__,5);$routes=(string)file_get_contents($root.'/app/zoosper-menu/config/api_routes.php');$controller=(string)file_get_contents($root.'/app/zoosper-menu/src/Api/MenuApiController.php');$services=(string)file_get_contents($root.'/app/zoosper-menu/config/services.php');
...
it('moves guarded item deletion out of Admin ownership',function():void{$root=dirname(__DIR__,5);expect($root.'/app/zoosper-menu/src/Application/MenuItemDeletionService.php')->toBeFile()->and($root.'/app/zoosper-menu/src/Admin/Lifecycle/MenuItemDeletionService.php')->not->toBeFile();});
