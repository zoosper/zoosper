...
final readonly class MenuAdminResponder {
 ...
 public function edit(AdminUser $user,int $id): Response{$menu=$this->menus->find($id);if($menu===null)return Response::html('Menu not found',404);return Response::html($this->views->render('Edit menu','zoosper-menu::admin/menu/edit.latte',['menu'=>$menu,'items'=>$this->menus->items($id),'sites'=>$this->choices->sites(),'pages'=>$this->choices->pages($menu->siteId),'saveUrl'=>$this->urls->url("menus/{$id}/edit"),'itemsUrl'=>$this->urls->url("menus/{$id}/items"),'deleteUrl'=>$this->urls->url("menus/{$id}/delete"),'token'=>$this->csrf->token()],$user,'menus'));}
 public function create(AdminUser $user): Response{return Response::html($this->views->render('Create menu','zoosper-menu::admin/menu/edit.latte',['menu'=>null,'items'=>[],'sites'=>$this->choices->sites(),'pages'=>[],'saveUrl'=>$this->urls->url('menus'),'itemsUrl'=>'','deleteUrl'=>'','token'=>$this->csrf->token()],$user,'menus'));}
