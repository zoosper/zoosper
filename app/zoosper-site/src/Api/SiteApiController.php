...
use Zoosper\Auth\Token\{PersonalAccessTokenAuthenticator,PersonalAccessTokenPrincipal};use Zoosper\Core\Http\{JsonResponder,Request,Response};use Zoosper\Site\Application\SiteMutationService;use Zoosper\Site\Lifecycle\SiteLifecycleCoordinator;use Zoosper\Site\Model\Site;use Zoosper\Site\Repository\SiteRepository;
final readonly class SiteApiController{
 public function __construct(private JsonResponder $json,private PersonalAccessTokenAuthenticator $auth,private SiteRepository $sites,private SiteLifecycleCoordinator $lifecycle,private SiteMutationService $mutations){}
 ...
 public function create(Request $r):Response{$p=$this->principal($r,'sites:write');if($p instanceof Response)return$p;try{$site=$this->mutations->create($r->json(),$p->user->id,$p->user->email);}catch(\RuntimeException $e){return$this->json->error('invalid_site',$e->getMessage(),422);}return$this->json->success(['site'=>$this->row($site)],201);}
 public function update(Request $r):Response{$p=$this->principal($r,'sites:write');if($p instanceof Response)return$p;$site=$this->site($r);if($site===null)return$this->json->error('site_not_found','Site does not exist.',404);try{$site=$this->mutations->update($site,$r->json(),$p->user->id,$p->user->email);}catch(\RuntimeException $e){return$this->json->error('invalid_site',$e->getMessage(),422);}return$this->json->success(['site'=>$this->row($site)]);}
