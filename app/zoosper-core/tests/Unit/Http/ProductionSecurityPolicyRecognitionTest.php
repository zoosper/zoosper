...
afterEach(function():void{foreach(['APP_ENV','SESSION_SECURE','RATE_LIMIT_ENABLED','RATE_LIMIT_MODE','RATE_LIMIT_IDENTITY_SALT'] as $key)putenv($key);putenv('APP_ENV=testing');});
it('rejects empty and unknown environments',function():void{putenv('APP_ENV');expect(fn()=>ProductionSecurityPolicy::assertEnvironment())->toThrow(RuntimeException::class);putenv('APP_ENV=prod');expect(fn()=>ProductionSecurityPolicy::assertEnvironment())->toThrow(RuntimeException::class);});
