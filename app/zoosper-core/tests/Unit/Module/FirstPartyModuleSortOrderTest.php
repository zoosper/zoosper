...
test('first-party module files do not declare arbitrary global sort order', function (): void {
    ...
    foreach (['app', 'packages'] as $directory) {
        foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($basePath . '/' . $directory)) as $file) {
            ...
            expect($module)->not->toHaveKey(
                'sort_order',
                'Runtime layer priority owns global module order; remove arbitrary sort_order from ' . $moduleFile,
            );
        }
    }
});
