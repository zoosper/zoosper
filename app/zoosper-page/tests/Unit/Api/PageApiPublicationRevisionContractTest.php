...
it('exposes PAT-scoped stateless Page publication and revision contracts through shared Page services', function (): void {
    $root=dirname(__DIR__,5); $routes=(string)file_get_contents($root.'/app/zoosper-page/config/api_routes.php'); $controller=(string)file_get_contents($root.'/app/zoosper-page/src/Api/PageApiController.php'); $wiring=(string)file_get_contents($root.'/app/zoosper-page/config/controllers.php');
...
it('keeps single Page publication orchestration outside Admin', function (): void {
    $root=dirname(__DIR__,5); expect($root.'/app/zoosper-page/src/Application/Publication/PagePublicationCoordinator.php')->toBeFile()->and($root.'/app/zoosper-page/src/Admin/Publication/PagePublicationCoordinator.php')->not->toBeFile();
});
