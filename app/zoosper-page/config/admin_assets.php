<?php

declare(strict_types=1);

return array (
  'assets' =>
  array (
    'zoosper-page-grid-search-script' =>
    array (
      'type' => 'script',
      'path' => '/asset/zoosper-page/js/page-grid-search.js?v=1ec0b66be94c',
      'sort_order' => 97,
      'attributes' =>
      array (
        'defer' => true,
      ),
    ),
    'zoosper-page-grid-workspace-style' =>
    array (
      'type' => 'style',
      'path' => '/asset/zoosper-page/css/page-grid-workspace.css?v=c0cbf59e1341',
      'sort_order' => 96,
    ),
    'zoosper-tag-selector-style' =>
    array (
      'type' => 'style',
      'path' => '/assets/admin/css/zoosper-tag-selector.css',
      'sort_order' => 40,
    ),
    'zoosper-tag-selector-script' =>
    array (
      'type' => 'script',
      'path' => '/assets/admin/js/zoosper-tag-selector.js',
      'sort_order' => 40,
      'defer' => true,
    ),
    'page-revision-pagination' =>
    array (
      'type' => 'script',
      'path' => '/assets/page/js/page-revision-pagination.js',
      'sort_order' => 260,
    ),
  ),
);










