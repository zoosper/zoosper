...
{
    public function __construct(private GridWorkspaceQuery $query)
    {
    }

    public function page(GridViewState $state, int $page): string
    {
        return $this->query->url(PageGridEndpointContract::VIEW_PATH, $state, page: $page);
    }

    public function sort(GridViewState $state, string $column, string $direction): string
    {
        return $this->query->url(
            PageGridEndpointContract::VIEW_PATH,
            $state,
            page: 1,
            sortBy: $column,
            sortDir: $direction,
    ...
    {
        return $this->query->url('/admin/pages/export', $state);
    }
}
