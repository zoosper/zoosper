...
- [ ] Reconcile the approved Fable Admin design language into Zoosper incrementally. The first bounded foundation keeps Admin as the sole semantic token owner, preserves fluid Light/Dark/Ocean behaviour, and explicitly excludes prototype-only navigation counts, fabricated data, hardcoded routes, and inline behaviour. Dashboard and Grid presentation follow as separately browser-accepted slices without weakening their existing ownership or persistence contracts.
