...
- Current development line: `0.3.2-alpha.1-dev`.
...
**`0.3.2-alpha.1-dev` is open.** The immutable annotated `v0.3.1-alpha.1` tag targets release commit `bfde15051396cd1dd19bdfaaa7b560e9c29e8287`. Continue from this new development baseline without changing the released source or retargeting its tag.
...
- [x] **[HIGH-03] Unified admin authentication rate limiting.** Complete. `AdminAuthenticationRateLimiter` is the single policy-execution boundary for HTML Admin login, API login, password-reset requests and 2FA challenges. The registered Admin middleware is now a thin HTML transport adapter. Disabled, report-only and enforcing modes, separate policy buckets, successful resets, opaque identity hashing, JSONL diagnostics and shared-bucket behaviour are protected by executable tests.
- [x] **SR-14D anonymous public session elimination.** Unmatched frontend GET and HEAD requests are classified as stateless before Application session bootstrap, while explicit Admin, login, CSRF, and pending-2FA routes retain their declared stateful policy.
- [x] **SR-14B multi-dimensional authentication throttling.** Password login and two-factor verification now enforce independent salted subject, client-IP, and subject-plus-IP buckets. Any denying dimension blocks in enforce mode, report-only mode records every dimension, missing client IP omits the shared IP bucket, and successful authentication preserves the shared IP abuse budget.
...
- [x] **[MED-01] Static analysis coverage & CI gate.** Complete. Psalm covers every first-party source-bearing package and runs as a blocking CI step; configuration-only `zoosper-session` remains covered by runtime, architecture, package and quality-gate contracts. Psalm rejects stale baseline entries and retains the reviewed baseline only as an explicit debt-reduction boundary on the path to zero.
- [x] **[MED-02] Dual templating / Latte role views.** Complete. Role Admin rendering now uses only the configured engine-independent template renderer and Auth-owned Latte templates; the controller-local `extract()`/`require` fallback and duplicate Admin-owned PHP Role views are removed.
...
- [x] **Automated secret generation and mandatory production boot validation:** Provided `security:generate-secrets` command (`bin/zoosper security:generate-secrets --write`), implemented canonical quoted/comment/`export` parsing shared with bootstrap, duplicate-target rejection, formatting-preserving checked atomic replacement with verified `0600` permissions, secret-redacted diagnostics, and fail-closed production boot checks for required keys/salts in `ProductionSecurityPolicy`.
...
- **[x] SR-14C deactivated-session revocation:** every fresh session-user resolution now rejects inactive Admin accounts and clears their authenticated session state immediately.
...
- [x] BR-2D: prove the latest immutable release upgrade against the MySQL production target.
