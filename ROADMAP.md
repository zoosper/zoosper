...
- [ ] **Real-Time Global Announcement Modal (future global-notifications workstream).** Give Super Admins a Settings surface to draft, publish, and unpublish a Global Announcement. Publishing must immediately notify currently active authenticated Admin users and must reconcile delivery for offline users after their next successful login. Each announcement opens as a mandatory one-time modal per user with an **Acknowledge** or **Close** action; persist acknowledgment by announcement and user so refreshes and later logins do not reopen it, while retaining the closed announcement in the future notifications list. Keep authoring and lifecycle mutations permission-gated, CSRF-protected, POST-only, audited, and safely escaped; make delivery idempotent and duplicate-safe. The database is authoritative and real-time transport is only an acceleration path, with authenticated reconnect/fallback and login/page-load reconciliation. Decide tenant/site targeting explicitly during design rather than inferring it.
