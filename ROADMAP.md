...
- [~] Reconcile the approved Fable Admin design language into Zoosper incrementally. The semantic foundation is complete; the current browser-review draft adds the compact shell/sidebar, secure account popover, Dashboard hierarchy/customisation surface, and package-owned Grid hierarchy in one bounded presentation pass. It excludes prototype-only navigation counts, fabricated data, hardcoded routes, and inline behaviour while preserving ownership and persistence contracts.
