...
Zoosper CMS is under active pre-release development. The latest tagged pre-release is `v0.3.0-alpha.1`; the supported development branch is `dev` on `v0.3.0-alpha.2`. No stable release has shipped. Security fixes are developed on `dev` and should be verified against the latest tagged pre-release and current `dev` branch as applicable.
