...
Zoosper CMS is under active pre-release development. The latest pre-release is `v0.3.1-alpha.1`; the supported development branch is `dev` on the `0.3.2-alpha.1-dev` development identity. No stable release has shipped. Security fixes are developed on `dev` and should be verified against the latest pre-release and current `dev` branch as applicable. Once tagged stable releases begin, this table will be updated to reflect which release lines receive security patches.
