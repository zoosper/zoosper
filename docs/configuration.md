...
- **Secrets & Encryption**: `APP_KEY`, `TWO_FACTOR_ENCRYPTION_KEY`, `RATE_LIMIT_IDENTITY_SALT`, and `CACHE_ENCRYPTION_KEY` can be generated and audited using `php bin/zoosper security:generate-secrets`. The command shares the canonical bootstrap parser, understands quoted values, inline comments and `export` prefixes, and preserves unrelated `.env` content. `--write` publishes through a same-directory temporary file, verifies mode `0600`, atomically replaces the destination, and fails closed on duplicate generated-secret keys or unverified writes. Audit and write diagnostics do not expose existing or generated secret values.
