...
Compact Grid controls provide explicit relationships to their filter and column panels, labelled dismiss controls, visible keyboard focus, mobile-safe panels, and reduced-motion treatment. Package-owned odd/even row styling, permanent row separators and a stronger header boundary improve horizontal record tracking across wide tables; hover and keyboard focus override the stripe, while selected rows retain the strongest fill plus a leading-edge marker and their checked selection control. The hierarchy is theme-aware and does not make colour the only selection signal.
