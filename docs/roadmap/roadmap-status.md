...
| 1.37c | Admin logout CSRF token hotfix | Admin logout form now includes the current CSRF token required by the central admin CSRF middleware. |
...
| Phase | Title |
|---|---|
| 1.37d | Login history writer verification if login-history still does not update after logout CSRF fix. |
| 1.37e | Editor.js image block integration backed by media assets. |
