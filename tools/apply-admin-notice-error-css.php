...
print "Zoosper admin notice error CSS restoration\n";
print "=========================================\n\n";

$target = find_notice_css_target($basePath);
if ($target === null) {
    fwrite(STDERR, "Unable to locate an admin CSS file.\n");
    exit(2);
}

$css = (string) file_get_contents($target);
if (str_contains($css, '.notice-error')) {
    print '- notice error CSS already exists in ' . relative_path($basePath, $target) . PHP_EOL;
    print "Result: OK\n";
    exit(0);
}
...
/* Phase 1.17.2: restore visible admin error notice styling. */
.notice.notice-error,
.notice-error {
    background: #fef3f2;
...
$backup = $target . '.phase-1.17.2.bak';
if (!is_file($backup)) {
    copy($target, $backup);
