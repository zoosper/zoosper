 ...
 *
 * Read-only guard to keep Zoosper tidy after the Page Momentum cleanup arc.
 *
 * Modes:
 * - report: observations/warnings only
 * - --strict: fail only on Page Momentum/process hygiene issues
 * - --global-strict: fail on all detected tool-pinning tests across the repo
 */
...
if ($pageMomentumToolPinningTests !== []) {
    $warnings[] = 'Page Momentum tests still appear to pin tool file paths: ' . count($pageMomentumToolPinningTests);
}
...
if ($strict || $globalStrict) {
    ...
    foreach ($pageMomentumToolPinningTests as $file) {
        $hardIssues[] = 'Page Momentum test pins tool path: ' . $file;
    }
}

if ($globalStrict) {
    foreach ($toolPinningTests as $file) {
        if (!in_array($file, $pageMomentumToolPinningTests, true)) {
...
if ($pageMomentumToolPinningTests !== []) {
    $report[] = '';
    $report[] = '### Page Momentum tests pinning tool paths';
    foreach ($pageMomentumToolPinningTests as $file) {
        $report[] = '- ' . $file;
    }
}
