...
- Added the SR-10 Media-owned Editor.js image picker browser flow with protected search and pagination, accessible dialog interaction, managed-image selection through the wrapper-scoped Editor insertion bridge, and executable browser/CI coverage.
- Established SR-9 Editor instance and block-insertion boundary: Editor.js instances are wrapper-scoped through an editor-owned WeakMap bridge, image insertion waits for readiness, uses the Blocks API, accepts only managed /media/ URLs, synchronises canonical content_json through save(), and is executable through npm, Composer, and CI gates without adding Media-to-Editor.js coupling.
- Closed SR-8 Admin Grid DOM behaviour coverage with a locked jsdom suite in Node's built-in test runner, exercising disclosure and ARIA state, keyboard column movement, locked boundaries, live table reflection, persistence-form synchronisation, dirty-state feedback, and page-size submission through npm, Composer, and CI gates; patched transitive nanoid and PostCSS versions are pinned through npm overrides and the high-severity audit gate is clean.
- Reconciled reviewer and release truth after SR-6: removed stale open claims for deployed account lockout, password reset, Composer prerelease constraints, Dashboard adoption, durable-tool ownership, root-tool pruning and Page Momentum retirement; corrected historical deployment wording; and repaired the malformed SR-6/MED-02 changelog line boundary.
- Closed SR-6 GenerateSecrets environment-file hardening: bootstrap and the command now share one canonical assignment parser; quoted values, inline comments and `export` prefixes are handled consistently; duplicate generated-secret keys fail before mutation; unrelated formatting and line endings are preserved; writes use checked same-directory temporary files, verified `0600` permissions and atomic replacement; and audit/write diagnostics redact secret values.
- Closed MED-02 Role Admin dual templating: made the configured template renderer fail-closed, removed controller-local `extract()` and `require`, and retired duplicate Admin-owned PHP Role views.
