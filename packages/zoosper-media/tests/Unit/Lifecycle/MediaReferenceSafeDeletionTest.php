...
function mediaReferenceDb(): PDO
{
    ...
    $pdo->exec('CREATE TABLE media_derivatives (id INTEGER PRIMARY KEY, media_asset_id INTEGER, profile TEXT, format TEXT, width INTEGER, height INTEGER, size_bytes INTEGER, storage_path TEXT, public_path TEXT, created_at TEXT, updated_at TEXT)');
...
it('blocks referenced deletion and permits archived unreferenced deletion', function (): void {
    ...
    $lifecycle = new MediaLifecycleCoordinator($pdo, $repository, new MediaStoredFileCleanupService(sys_get_temp_dir()), new MediaDerivativeRepository($pdo, sys_get_temp_dir()), new MediaReferenceInspector($pdo));
