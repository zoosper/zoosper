...
final readonly class MediaVisualGridWorkspace
{
 ...
 public function render(int $adminUserId,Request $request,string $action,string $csrf):string{$definition=$this->source->definition();$state=$this->states->resolve($adminUserId,MediaGridSource::KEY,$definition,AdminCollectionGridQuery::values($request,$definition),AdminCollectionGridQuery::bookmark($request));$controlState=new GridViewState($this->orderer->apply($definition,$state->columnOrder),$state->criteria,$state->visibleColumns,$state->columnOrder,$state->bookmarks,$state->activeBookmarkId);$page=$this->source->paginate($state->criteria);return $this->controls->render($controlState,$action).$this->cards->render($page,$state->criteria,$state->visibleColumns,$csrf);}
}
