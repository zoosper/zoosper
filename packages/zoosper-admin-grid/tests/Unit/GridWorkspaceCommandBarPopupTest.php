...
it('anchors the popup to its trigger and preserves escape closure', function (): void {
    $root = dirname(__DIR__, 4);
    ...
    expect($script)->not->toBeFalse()
        ...
        ->and($script)->toContain("settingsClose?.addEventListener('click'")
        ->and($script)->toContain("event.key === 'Escape'")
        ->and($script)->toContain('settingsToggle?.focus()');
});

it('publishes the current aligned popup assets', function (): void {
    $root = dirname(__DIR__, 4);
