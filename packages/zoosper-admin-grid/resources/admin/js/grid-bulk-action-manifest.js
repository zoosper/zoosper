(() => {
    ...
    const bind = () => {
        ...
        const supported = manifest.actions.filter((definition) => definition
            && definition.selectionScope === 'explicit_identities'
            && ((definition.executionType === 'client_download' && definition.id === 'export.selected')
                || (definition.executionType === 'server_mutation' && definition.id === 'page.publish')));
        action.dataset.gridManifestBound = 'true';
        action.replaceChildren(new Option('Bulk actions', ''));
        supported.forEach((definition) => action.add(new Option(String(definition.label), String(definition.id))));
    };
    bind();
