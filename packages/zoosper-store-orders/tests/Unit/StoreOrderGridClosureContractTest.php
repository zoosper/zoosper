...
it('closes enabled Store Orders workspace persistence through CSRF protected POST coordination', function (): void {
    StoreOrdersTestEnvironment::enabled(function (): void {
        ...
        $source = file_get_contents($root . '/packages/zoosper-store-orders/src/Admin/StoreOrderAdminController.php');
        expect($routes)->toContain([
            'method' => 'POST', 'path' => '/admin/store-orders',
            'controller' => StoreOrderAdminController::class,
            'action' => 'mutate', 'permission' => 'store_order.view',
...
it('keeps Store Orders mutation identity server owned', function (): void {
    $root = dirname(__DIR__, 4);
    $source = file_get_contents($root . '/packages/zoosper-store-orders/src/Admin/StoreOrderGridMutationHandler.php');
