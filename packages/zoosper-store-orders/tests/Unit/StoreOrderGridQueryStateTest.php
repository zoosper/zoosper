...
it('nests only approved result filters for the shared workspace resolver', function (): void {
    $state = StoreOrderGridQueryState::fromQuery([
        'store_code' => '49', 'kiosk_website_id' => '55',
        'order_id' => '145201973', 'customer' => 'Sample Customer', 'status' => 'Charged',
        'placed_from' => '2026-07-01', 'placed_to' => '2026-08-02',
        'page' => '2', 'page_size' => '50', 'sort' => 'order_date', 'dir' => 'desc',
    ...
    expect($state['filters'])->toBe([
        ...
        ->and($state['filters'])->not->toHaveKeys(['store_code', 'kiosk_website_id', 'unexpected']);
});
...
it('makes the controller pass normalised query state into the workspace', function (): void {
    $source = file_get_contents(dirname(__DIR__, 4) . '/packages/zoosper-store-orders/src/Admin/StoreOrderAdminController.php');
